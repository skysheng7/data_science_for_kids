...
```{r}
mcdonalds <- read_csv("../datasets/menu_data.csv")
head(mcdonalds, 5)
dim(mcdonalds)
```
...
# acc <- validation |>
#   accuracy(truth = Category, estimate = ...) |>
#   pull(.estimate)
...
run_model <- function(k_value, train_data, test_data) {
      ...
      acc <- validation |>
            mutate(
              ...
              Category_pred = as.factor(Category_pred)
            ) |>
            accuracy(truth = Category, estimate = Category_pred) |>
            pull(.estimate)
