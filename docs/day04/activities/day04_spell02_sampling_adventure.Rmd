...
sampling_results <- rep_sample_n(magical_creatures, size = 40, reps = 100) %>%
  group_by(replicate) %>%
...
for (size in sample_sizes) {
  ...
  all_results <- all_results %>%
    mutate(sample_size = factor(sample_size, levels = c("Size 20", "Size 40", "Size 100")))
