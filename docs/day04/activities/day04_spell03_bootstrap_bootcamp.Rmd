---
title: "🔮 Day 4 - Spell 3: Bootstrap Bootcamp - The Great Dragon Fire Sheep Rescue"
...
# Create our population of 1000 magical sheep
# 600 black sheep (60%) and 400 white sheep (40%)
magical_sheep <- tibble(
  sheep_id = 1:1000,
  sheep_color = factor(c(rep("black", 600), rep("white", 400)))
)
...
ggplot(bootstrap_results, aes(x = black_proportion)) +
  ...
  geom_vline(xintercept = original_black_prop, color = "red", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = true_black_proportion, color = "blue", linetype = "dashed", linewidth = 1) +
  labs(title = "🔮 Bootstrap Distribution: 1000 Magic Resamples",
       x = "Bootstrap Sample Proportion of Black Sheep", 
       y = "Count",
       subtitle = "Red line: Our original estimate, Blue line: True proportion") +
  theme_minimal()
...
ggplot(bootstrap_results, aes(x = black_proportion)) +
  ...
  geom_vline(xintercept = confidence_interval[1], color = "green", linetype = "solid", linewidth = 2) +
  geom_vline(xintercept = confidence_interval[2], color = "green", linetype = "solid", linewidth = 2) +
  geom_vline(xintercept = true_black_proportion, color = "red", linetype = "dashed", linewidth = 1) +
  annotate("rect", xmin = confidence_interval[1], xmax = confidence_interval[2], 
           ymin = 0, ymax = Inf, alpha = 0.2, fill = "green") +
  labs(title = "🏆 Our 90% Confidence Prophecy",
       x = "Bootstrap Sample Proportion of Black Sheep", 
       y = "Count",
       subtitle = "Green area: Our 90% confidence interval, Red line: True proportion") +
  theme_minimal()
```
...
ggplot(ci_results, aes(x = factor(sample_size))) +
  geom_linerange(aes(ymin = ci_lower, ymax = ci_upper), linewidth = 3, color = "blue") +
  geom_hline(yintercept = true_black_proportion, color = "red", linetype = "dashed") +
  labs(title = "🎯 Magic of Sample Size: Confidence Intervals Get Narrower!",
       x = "Sample Size",
       y = "90% Confidence Interval",
       subtitle = "Red line: True proportion. Blue lines: Confidence intervals") +
  theme_minimal()
```

## 🎯 Key Magical Discoveries!

**🎩 Discovery 1: The Magic Hat Trick (Bootstrap)**
...
**🎯 Discovery 2: Confidence Prophecies (Confidence Intervals)**
...
**🔍 Discovery 3: More Sheep = Better Prophecies**
...
**⚡ Discovery 4: Magic Hat vs. Real Field Samples**
...
**🗳️ Election Polls:** When news says "Candidate A leads by 5% with a margin of error of ±3%", that's a confidence interval from bootstrapping!

**⚕️ Medical Research:** Doctors use bootstrap confidence intervals to estimate how effective a new treatment is.

**🎮 Game Analytics:** Game companies bootstrap user data to estimate what percentage of players will like a new feature.

**📱 App Reviews:** When you see "4.2 stars (±0.3)", that confidence interval came from bootstrap magic!

**🌡️ Climate Science:** Scientists use bootstrapping to estimate temperature trends and their uncertainty ranges.